...
export function lookupByBic11(bic11: string): BICRow | null {
  if (!stmtByBic11) {
    stmtByBic11 = getDB().prepare('SELECT * FROM bic_entries WHERE bic11 = ? LIMIT 1');
  }
...
export function lookupByBic8(bic8: string): BICRow[] {
  if (!stmtByBic8) {
    stmtByBic8 = getDB().prepare('SELECT * FROM bic_entries WHERE bic8 = ?');
  }
...
export function getEntryCount(): number {
  return (getDB().prepare('SELECT COUNT(*) as cnt FROM bic_entries').get() as { cnt: number }).cnt;
}
