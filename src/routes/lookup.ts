...
import { recordLookup } from '../lib/stats.js';
...
lookupRoute.get('/v1/bic/:code', (c) => {
  ...
  recordLookup(validation.country_code ?? null, found, COST_USDC);
  ...
  if (!found) {
    result.note = 'BIC format valid but not found in database. Data sourced from GLEIF — coverage may be partial.';
  }
